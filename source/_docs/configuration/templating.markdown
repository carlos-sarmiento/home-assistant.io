...
- `relative_time` converts datetime object to its human-friendly "age" string. The age can be in second, minute, hour, day, month or year (only the biggest unit is considered, e.g.,  if it's 2 days and 3 hours, "2 days ago" or "in 2 days" will be returned, depending on the date difference). It works for dates in the past and the future.
